...
import { getActiveSubscription, getCurrentAndUpcomingSubscriptions, getPaidSubscriptionHistory } from "@/lib/db/subscriptions"
...
export default async function Page() {
  ...
  const membershipTiming = subscription
    ...
    : null
  const progress = membershipTiming?.progressRemaining ?? 0
  const statusVariant = membershipTiming?.status === "critical" || membershipTiming?.status === "expired"
    ? "destructive"
    : membershipTiming?.status === "soon"
      ? "warning"
      : "success"
  const isPremium = Boolean(
    subscription
    && subscription.membership_plans?.code !== "DAILY_PASS"
  )
  ...
  return (
    <div className="space-y-3 sm:space-y-6">
      ...
      {subscription && membershipTiming?.isExpiringSoon ? (
        <div className={`rounded-xl border p-3 text-sm sm:p-4 ${
          membershipTiming.isCritical
          ...
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              <p className="font-semibold">
                {membershipTiming.alertTitle}
              </p>
              <p className="mt-1">
                {planName} aktif sampai {formatDate(subscription.end_date)} ({membershipTiming.summaryLabel}).
              </p>
            </div>
            <Link href="/member/billing">
              <Button variant="outline" size="sm" className={membershipTiming.isCritical ? "border-red-500/50 text-red-700 hover:bg-red-500/10 dark:text-red-300" : ""}>
                Perpanjang Sekarang
              </Button>
            </Link>
          </div>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 gap-2 p-3 pt-0 sm:grid-cols-2 sm:p-6 sm:pt-0 xl:grid-cols-4">
          ...
          <CompactInfo
            icon={CalendarCheck2}
            label="Masa Aktif"
            value={membershipTiming?.remainingLabel || "-"}
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              {subscription ? (
                <Badge variant={statusVariant}>
                  {membershipTiming?.statusLabel || "Aman untuk latihan"}
                </Badge>
              ) : (
              ...
              <>
                <div className="max-w-xl">
                  <div className="mb-2 flex justify-between text-sm">
                    <span className={membershipTiming?.isCritical ? "font-semibold text-red-600 dark:text-red-400" : ""}>
                      {planName}
                    </span>
                    <span className={membershipTiming?.isCritical ? "font-semibold text-red-600 dark:text-red-400" : ""}>
                      {membershipTiming?.summaryLabel || "-"}
                    </span>
                  </div>
                  <Progress
                    value={progress}
                    className={membershipTiming?.isCritical ? "bg-red-200 [&>div]:bg-red-500" : membershipTiming?.status === "soon" ? "bg-amber-200 [&>div]:bg-amber-500" : ""}
                  />
                ...
                {membershipTiming?.isExpiringSoon ? (
                  <Link href="/member/billing">
                    <Button className="w-full" variant={membershipTiming.isCritical ? "destructive" : "default"}>
