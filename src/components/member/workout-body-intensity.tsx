      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="rounded-lg border border-border bg-background/80 p-2">
          <Model
            ...
            onClick={(stats) => setSelected(findAreaByMuscle(data, stats.muscle))}
            style={{ width: "100%", height: "17rem", padding: "0.25rem" }}
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {topAreas.map((item) => (
            <button
              ...
              className="rounded-lg border border-border p-3 text-left text-sm transition hover:border-primary/50 hover:bg-muted/40"
            >
              <div className="flex items-center justify-between gap-2">
                <span className="font-medium">{item.label}</span>
                <span className="tabular-nums text-muted-foreground">{item.count} set</span>
              </div>
